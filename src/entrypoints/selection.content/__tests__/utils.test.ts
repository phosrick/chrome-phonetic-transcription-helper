...
describe("buildContextSnapshot", () => {
  ...
  it("filters out phonetic <rt> tags from the surrounding context text", () => {
    document.body.innerHTML = `
      <article>
        <p id="paragraph">
          Alpha <ruby>hello <rt>həˈloʊ</rt></ruby><strong id="selection">world</strong>
        </p>
      </article>
    `
    ...
    expect(buildContextSnapshot(createSelectionSnapshot(range, "world"))).toEqual({
      text: "Alpha hello world",
      paragraphs: ["Alpha hello world"],
    })
  })
})
...
describe("readSelectionSnapshot", () => {
  ...
  it("filters out phonetic <rt> tags from the selected range text", () => {
    document.body.innerHTML = `
      <div id="selection">
        <ruby>hello<rt>həˈloʊ</rt></ruby> world
      </div>
    `
