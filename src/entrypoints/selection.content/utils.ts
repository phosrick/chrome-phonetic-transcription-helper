...
export function getTextFromRangeWithoutRt(range: Range): string {
  const fragment = range.cloneContents()
  const rts = fragment.querySelectorAll("rt")
  rts.forEach(rt => rt.remove())
  return fragment.textContent ?? ""
}

export function readSelectionSnapshot(selection: Selection | null): SelectionSnapshot | null {
  if (!selection || normalizeSelectedText(selection.toString()) === "") {
    return null
  }
  ...
  for (const rangeSnapshot of ranges) {
    try {
      const liveRange = toLiveRange(rangeSnapshot)
      textParts.push(getTextFromRangeWithoutRt(liveRange))
    }
    catch {
      // Fallback if live range fails to construct
    }
  }

  const text = normalizeSelectedText(textParts.join(" "))
  if (text === "") {
    return null
  }
