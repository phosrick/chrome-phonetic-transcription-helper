...
export function translationMessage() {
  onMessage("getEnablePageTranslationByTabId", async (msg) => {
    ...
    return await getPageTranslationState(tabId)
  })
  ...
  onMessage("tryToSetEnablePageTranslationByTabId", async (msg) => {
    const { tabId, enabled, mode, analyticsContext } = msg.data
    if (!enabled) {
      await setPageTranslationEnabled(tabId, false, undefined, mode)
      notifyPageTranslationStateChanged(tabId, false, mode)
    }
    requestManagerToTogglePageTranslation(tabId, enabled, mode, analyticsContext)
  })
  ...
  onMessage("setAndNotifyPageTranslationStateChangedByManager", async (msg) => {
    const tabId = msg.sender?.tab?.id
    const { enabled, url, mode } = msg.data
    if (typeof tabId === "number") {
      ...
      if (enabled && isIframe(senderFrameId)) {
        ...
        notifyPageTranslationStateChanged(tabId, true, currentState.mode)
        return
      }

      await setPageTranslationEnabled(tabId, enabled, url ?? msg.sender?.tab?.url, mode)
      notifyPageTranslationStateChanged(tabId, enabled, mode)
